...
export default function ClientPage() {
    ...
    const predictWebcam = async () => {
        ...
        if (video && canvas && landmarker && har) {
            ...
            if (lastVideoTimeRef.current !== video.currentTime) {
                ...
                // Process Logic
                if (!isRestingRef.current && result.landmarks && result.landmarks.length > 0) {
    ...
    useEffect(() => {
        ...
        const isMatchingExercise = stats.exercise && stats.exercise.toLowerCase().includes(currentTarget.name.split(' ')[0].toLowerCase());
            ...
            <div className="min-h-screen bg-white text-zinc-900 flex flex-col items-center justify-center p-4">
                ...
                <div className="bg-white p-8 rounded-xl border border-zinc-200 text-center shadow-xl">
                    ...
                    {isSaving ? <p className="text-blue-600">Saving...</p> : <p className="text-zinc-400">Saved.</p>}
                     <button onClick={() => window.location.reload()} className="mt-8 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Start New Workout</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-6 font-sans selection:bg-primary/30">
            <header className="flex justify-between items-center mb-8">
                 <h1 className="text-3xl font-light tracking-widest text-zinc-800">STRAPS<span className="font-bold text-primary">.AI</span></h1>
