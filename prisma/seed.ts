...
async function main() {
  ...
  const coach1 = await prisma.users.create({
    data: {
      id: "C00001", // Explicit String ID for testing
      name: 'Coach One',
      role: 'COACH',
    },
  });
  ...
  const coach2 = await prisma.users.create({
    data: {
      id: "C00002",
      name: 'Coach Two',
      role: 'COACH',
  ...
  const client3 = await prisma.users.create({
    data: {
      id: "U00003",
      name: 'Client Three',
      role: 'CLIENT',
